...
TEST(TestMemoryController, TestMemoryControllerChannelReadOperation) 
{
    auto testPassed = false;
    MemoryController memController;
    shared_ptr<Memory> rom(new ROM(0x100));
...
TEST(TestMemoryController, TestMemoryControllerChannelWriteOperation) 
{
    auto testPassed = false;
    MemoryController memController;
    shared_ptr<Memory> rom(new ROM(0x100));
    ...
    aluChannel->OnReceived([&testPassed, &rom](MemoryMessage message) -> void 
    {
        ...
        {
            if (auto result = rom->Read(0x0010, MemoryAccessType::Byte);
                get<uint8_t>(result) == 0xBB)
                testPassed = true;
        }
    });
