...
TEST(CoreTests_OpenGLVideoOutput, TestSetBackgroundSetBackgroundTiles) 
{
    ...
    uint8_t backgroundMap[] = {0x00, 0x01, 0x00, 0x01, 0x00, 0x01, 0x00, 0x01, 0x00, 0x01, 0x00, 0x01, 0x00, 0x01, 0x00, 0x01, 
    ...
    for (auto j = 0llu; j < gbxcore::constants::DMGBCMaxBackgroundHorizontalTileCount; ++j)
        for (auto i = 0llu; i < gbxcore::constants::DMGBCMaxBackgroundHorizontalTileCount; ++i)
        {
            // i: columns
            // j: lines
            auto pixelCounter = i * 8 + 
                                j * gbxcore::constants::DMGBCMaxBackgroundHorizontalTileCount * 8 * 8;
            
            cout << "Tile: " << (i + j * gbxcore::constants::DMGBCMaxBackgroundHorizontalTileCount) << '\n';

            for (auto k = 0; k < 8; ++k)
            {
                for (auto l = 0; l < 8; ++l)
                {
                    if ((i + j * gbxcore::constants::DMGBCMaxBackgroundHorizontalTileCount)%2 == 0)
                    {
